...
export async function GET(request: NextRequest) {
  ...
  if (!projectId || !agentId) {
    return new Response('Missing projectId or agentId', { status: 400 })
  }

  // Create a unique key for this subscription
  const subscriptionKey = `${projectId}:${agentId}:${campaignIds.join(',')}`
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      console.log(`[SSE] New connection for ${subscriptionKey}`)
      ...
      const keepAlive = setInterval(() => {
        try {
          ...
          console.log('[SSE] Keep-alive failed, cleaning up')
