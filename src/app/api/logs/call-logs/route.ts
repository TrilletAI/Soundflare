...
export async function OPTIONS(request: NextRequest) {
  return new NextResponse(null, {
    ...
    headers: {
      ...
      'Access-Control-Allow-Headers': 'Content-Type, x-soundflare-token',
    },
  });
}
...
export async function POST(request: NextRequest) {
  try {
    const token = request.headers.get('x-soundflare-token');
    ...
    // 🤖 AUTO AI REVIEW: Trigger automatic AI review if enabled for agent
    try {
      if (insertedLog?.id && agent_id) {
        ...
        const autoReviewEnabled = agentData?.auto_review_enabled ?? true; // Default to true if not set
        ...
        if (autoReviewEnabled) {
          ...
          if (queueError) {
            console.error('⚠️ Failed to create pending review record:', queueError);
          }
          
          // Fire-and-forget: Trigger AI review webhook asynchronously
          // This returns immediately without blocking the response
          triggerAIReviewFireAndForget(insertedLog.id, agent_id);
        } else {
          console.log(`⏭️ AI Review skipped (disabled for agent): ${agent_id}`);
        }
      }
    } catch (reviewError) {
      // Don't fail the entire request if review triggering fails
      console.error('⚠️ Failed to trigger AI review (non-blocking):', reviewError);
    }
