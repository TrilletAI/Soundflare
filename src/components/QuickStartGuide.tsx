...
function QuickStartGuide({ agentId }: { agentId: string }) {
    ...
    const fullExampleCode = `from soundflare import LivekitObserve
...
if __name__ == "__main__":
    ...
    const steps = [
      {
        id: 'install',
        title: 'Install SoundFlare',
        subtitle: 'One command to get started',
        code: 'pip install soundflare',
        icon: '📦'
      },
      {
        id: 'credentials',
        ...
        items: [
          ...
          { text: 'Use your SoundFlare Project API key which you saved earlier', status: 'ready' }
        ]
      },
      {
        id: 'env',
        ...
        code: `TRILLET_EVALS_API_KEY=your_trillet_evals_api_key_here`,
        icon: '⚙️'
      },
      {
        id: 'integration',
        ...
        integrationSteps: [
          {
            title: '1. Import SoundFlare',
            code: `from soundflare import LivekitObserve
import os`
          },
          {
            title: '2. Initialize SoundFlare',
            code: `trillet_evals = LivekitObserve(
    agent_id="${agentId}",
    apikey=os.getenv("TRILLET_EVALS_API_KEY")
)`
          },
...
session_id = trillet_evals.start_session(session=session)
        ...
        <div className="flex-1 w-full overflow-y-auto">
          <div className="p-8">
            <div className="space-y-8">
              {steps.map((step, index) => (
                <div key={step.id} className="bg-white rounded-xl border border-neutral-200 p-6 relative">
                  {/* Completion Toggle */}
                  <button
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex flex-col items-center">
                      <div className="w-12 h-12 bg-orange-500 rounded-xl flex items-center justify-center text-white mb-2">
                          ...
                          {step.items.map((item, itemIndex) => (
                            <div key={itemIndex} className="flex items-center gap-3">
                              <div className={`w-4 h-4 rounded-full flex items-center justify-center ${
                                ...
                                item.status === 'ready' ? 'bg-orange-500' : 'bg-orange-500'
                              }`}>
                                {item.status === 'done' ? (
                      ...
                      {step.integrationSteps && (
                        <div className="space-y-4 mb-6">
                          {step.integrationSteps.map((integrationStep, stepIndex) => (
                            <div key={stepIndex} className="border border-neutral-100 rounded-lg p-4">
                          ...
                          <div className="mt-6 border-t border-neutral-100 pt-6">
                            <button
                              ...
                              className="flex items-center gap-2 text-orange-600 hover:text-orange-700 text-sm font-medium mb-4"
                            >
