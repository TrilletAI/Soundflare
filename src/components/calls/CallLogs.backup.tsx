...
import { Phone, Clock, CheckCircle, XCircle, Loader2, AlertCircle, RefreshCw, Sparkles, Inbox, Eye, EyeOff, PanelLeftClose, PanelLeft, X } from "lucide-react"
...
const CallLogs: React.FC<CallLogsProps> = ({ project, agent, onBack, isLoading: parentLoading }) => {
  ...
  // NEW: Field Discovery hook - must be called unconditionally
              ...
              {filterGroups.map(group =>
                group.filters.map(filter => (
                  <Badge
                    ...
                    variant="secondary"
                    className="gap-1.5 pl-2.5 pr-1.5 py-1 text-xs font-normal bg-blue-50 dark:bg-orange-950/30 text-blue-700 dark:text-orange-300 border border-blue-200 dark:border-orange-700 hover:bg-blue-100 dark:hover:bg-orange-950/50 transition-colors"
                  >
                    ...
                    <button
                      ...
                      className="flex items-center justify-center w-3.5 h-3.5 rounded-sm hover:bg-blue-200 dark:hover:bg-orange-800 transition-colors"
                ...
                <TableHeader className="sticky top-0 z-10 bg-background/95 dark:bg-neutral-900/95 backdrop-blur-sm border-b-2">
                  <TableRow className="bg-muted/80 dark:bg-neutral-800/80 hover:bg-muted/80 dark:hover:bg-neutral-800/80">
                    ...
                    {visibleColumns.metadata.map((key) => (
                      <TableHead
                        key={`metadata-${key}`}
                        className="w-[200px] font-semibold text-foreground dark:text-gray-100 bg-blue-50/50 dark:bg-orange-950/20 border-r border-blue-200/50 dark:border-orange-800/50"
                      >
                    ...
                    {visibleColumns.transcription_metrics.map((key, index) => (
                      <TableHead
                        ...
                        className={cn(
                          "w-[200px] font-semibold text-foreground dark:text-gray-100 bg-blue-50/50 dark:bg-orange-950/20",
                          index === 0 && visibleColumns.metadata.length === 0 && "border-l-2 border-primary/30 dark:border-primary/40",
                          index < visibleColumns.transcription_metrics.length - 1 && "border-r border-blue-200/50 dark:border-orange-800/50"
                        )}
                      >
                        <div className="flex flex-col">
                          <span className="text-sm">{key}</span>
                    ...
                    >
                      ...
                      {visibleColumns.metadata.map((key) => (
                        <TableCell
                          ...
                          className="py-4 bg-blue-50/30 dark:bg-orange-950/10 border-r border-blue-200/50 dark:border-orange-800/50"
                        >
                          <DynamicJsonCell
                            data={call.metadata}
                            fieldKey={key}
                      ...
                      {visibleColumns.transcription_metrics.map((key, index) => (
                        <TableCell
                          ...
                          className={cn(
                            "py-4 bg-blue-50/30 dark:bg-orange-950/10",
                            index === 0 && visibleColumns.metadata.length === 0 && "border-l-2 border-primary/30 dark:border-primary/40",
                            index < visibleColumns.transcription_metrics.length - 1 && "border-r border-blue-200/50 dark:border-orange-800/50"
                          )}
                        >
