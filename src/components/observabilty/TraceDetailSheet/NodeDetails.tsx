    ...
    return (
      <div className="space-y-4">
            ...
            <>
              ...
              <div className="space-y-3">
                <h4 className="font-medium text-sm flex items-center gap-2 text-gray-900 dark:text-gray-100">
                  ...
                  {selectedStage.id === "stt" && trace.stt_metrics?.audio_duration && (
                    <span className="text-xs text-gray-500 dark:text-gray-400 ml-auto">
                      Audio stream •{" "}
                      <code className="bg-gray-100 dark:bg-neutral-800 text-gray-800 dark:text-gray-200 px-1 rounded">{trace.stt_metrics.audio_duration.toFixed(1)}s</code>
                    </span>
                  )}
                  {selectedStage.id === "tts" && trace.tts_metrics?.audio_duration && (
                    <span className="text-xs text-gray-500 dark:text-gray-400 ml-auto">
                      Text length:{" "}
                      <code className="bg-gray-100 dark:bg-neutral-800 text-gray-800 dark:text-gray-200 px-1 rounded">{trace.agent_response?.length || 0} chars</code>
                    </span>
                  )}
                </h4>
                {selectedStage.id === "stt" && (
                  <div className="space-y-3">
                    <div className="bg-gray-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-3 text-sm text-gray-600 dark:text-gray-400 italic">
                ...
                {selectedStage.id === "llm" && (
                  <div className="bg-blue-50 dark:bg-orange-900/20 border-l-4 border-blue-500 dark:border-orange-400 pl-3 py-2 text-sm text-gray-900 dark:text-gray-100">
          ...
          <div className="bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/10 dark:to-indigo-900/10 border border-purple-200 dark:border-purple-800 rounded-xl p-5 mb-6">
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {trace.enhanced_data.prompt_data.available_tools.map((tool: any, idx: number) => (
                    <div key={idx} className="bg-blue-50 dark:bg-orange-900/20 border border-blue-200 dark:border-orange-800 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-1">
                        <code className="text-sm font-semibold text-blue-800 dark:text-orange-200">{tool.name}</code>
                        <span className="text-xs text-blue-600 dark:text-orange-400 bg-blue-100 dark:bg-orange-800 px-2 py-1 rounded">{tool.tool_type}</span>
                      </div>
            ...
            {trace.enhanced_data.prompt_data.conversation_history?.length > 0 && (
              <div>
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto border border-neutral-200 dark:border-neutral-700 rounded-lg p-3 bg-white dark:bg-neutral-800">
                  {trace.enhanced_data.prompt_data.conversation_history.map((message: any, idx: number) => {
                    ...
                    const roleColors = {
                      ...
                      user: "bg-blue-50 dark:bg-orange-900/20 border-blue-200 dark:border-orange-800 text-blue-800 dark:text-orange-200",
                      assistant: "bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-800 dark:text-green-200",
                      unknown: "bg-gray-50 dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 text-gray-600 dark:text-gray-400",
                    }
        ...
        {selectedStage.id === "llm" && selectedStage.tools && selectedStage.tools.length > 0 && (
          <div className="bg-blue-50 dark:bg-orange-900/20 border-l-4 border-blue-500 dark:border-orange-400 rounded-lg p-4">
            <h4 className="font-medium text-sm mb-3 flex items-center gap-2 text-gray-900 dark:text-gray-100">
              <Wrench className="w-4 h-4 text-blue-600 dark:text-orange-400" />
              Tool Executions ({selectedStage.tools.length})
            </h4>
            <div className="space-y-3">
              {selectedStage.tools.map((tool: any, index: number) => (
                <div key={index} className="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg p-3 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded bg-blue-100 dark:bg-orange-800 flex items-center justify-center">
                        <Wrench className="w-3 h-3 text-blue-600 dark:text-orange-400" />
                      </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <code className="text-xs bg-gray-100 dark:bg-neutral-800 text-gray-800 dark:text-gray-200 px-2 py-1 rounded flex-1 break-all">
        ...
        {selectedStage.metrics && (
          <div className="bg-blue-50 dark:bg-orange-900/20 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {Object.entries(selectedStage.metrics).map(([key, value]) => (
                <div key={key} className="bg-white dark:bg-neutral-800 rounded p-3 border border-neutral-200 dark:border-neutral-700">
        ...
        {selectedStage.config && (
          <div className="bg-gray-50 dark:bg-neutral-800 rounded-lg p-4">
            ...
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded p-3">
              <div className="grid grid-cols-1 gap-2 text-sm">
                {selectedStage.id === "vad" && (
                  <>
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Activation Threshold:</span>
                      <code className="bg-gray-100 dark:bg-neutral-800 text-gray-800 dark:text-gray-200 px-1 rounded text-xs">
                ...
                {selectedStage.id === "stt" && (
                  <>
                    <div className="flex justify-between">
                      ...
                      <code className="bg-gray-100 dark:bg-neutral-800 text-gray-800 dark:text-gray-200 px-1 rounded text-xs">{selectedStage.config.model}</code>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Language:</span>
                      <code className="bg-gray-100 dark:bg-neutral-800 text-gray-800 dark:text-gray-200 px-1 rounded text-xs">{selectedStage.config.language}</code>
                    </div>
                    <div className="flex justify-between">
                ...
                {selectedStage.id === "llm" && (
                  <>
                    ...
                    <div className="flex justify-between">
                      ...
                      <code className="bg-gray-100 dark:bg-neutral-800 text-gray-800 dark:text-gray-200 px-1 rounded text-xs">{selectedStage.config.temperature}</code>
                    </div>
                    {selectedStage.metrics && (
                      <div className="flex justify-between">
                        <span className="text-gray-600 dark:text-gray-400">Prompt Tokens:</span>
                        <code className="bg-gray-100 dark:bg-neutral-800 text-gray-800 dark:text-gray-200 px-1 rounded text-xs">{selectedStage.metrics.prompt_tokens}</code>
                      </div>
                    )}
                    {selectedStage.metrics && (
                      <div className="flex justify-between">
