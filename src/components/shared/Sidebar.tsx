...
export default function Sidebar({ 
  ...
  useEffect(() => {
    ...
    if (shouldAutoCollapse && !isCollapsed && !hasAutoCollapsed) {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('soundflare-sidebar-collapsed', JSON.stringify(true))
      }
    }
  ...
  const groupedNavigation = (): NavigationGroup[] => {
    ...
    const groupOrder = [
      'agents', 'logs', 'reports', 'analytics', 'integrations',
      'team', 'settings', 'configuration', 'batch calls', 'project settings', 'resources'
    ]
    ...
    groupOrder.forEach(groupId => {
      if (groups[groupId]) {
        ...
        const newGroup: NavigationGroup = {
          id: groupId,
          name: groupId === 'logs' ? 'Monitoring' :
                groupId === 'batch calls' ? 'Batch Calls' :
                groupId === 'project settings' ? 'Project Settings' :
                groupId.charAt(0).toUpperCase() + groupId.slice(1),
          items: [...groupItems] // Create a copy to modify if needed
        }

        // Special handling for 'agents' group on agent detail page
        if (groupId === 'agents' && config.type === 'agent-detail' && config.context?.agentName && config.context?.agentId && config.context?.projectId) {
          newGroup.items.push({
            id: config.context.agentId,
            name: config.context.agentName,
            icon: 'Bot',
            path: `/${config.context.projectId}/agents/${config.context.agentId}`,
            group: 'agents',
            isChild: true
          })
        }
  ...
  const renderNavigationItem = (item: NavigationItem) => {
    ...
    const content = (
      <div className={`
        ...
        ${isActive
          ? 'bg-gray-100 dark:bg-neutral-800 text-gray-900 dark:text-gray-100 border border-neutral-300 dark:border-neutral-700'
        ...
        ${item.isChild ? 'ml-6 text-xs py-1.5' : ''}
      `}>
        <Icon className={`flex-shrink-0 ${isActive ? 'text-gray-700 dark:text-gray-200' : 'text-gray-500 dark:text-gray-400'} ${item.isChild ? 'w-3.5 h-3.5' : 'w-4 h-4'}`} />
    ...
    <>
      ...
      `}>
            ...
            {(!isCollapsed || isMobile) && (
              ...
              >
                ...
                <div className="min-w-0">
                  <h1 className="text-sm font-semibold text-gray-900 dark:text-gray-100 group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors truncate">
                    SoundFlare
                  </h1>
                </div>
              </Link>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          ...
          // Check if we should indent this group (agent detail page + not Agents or Project Settings)
          const shouldIndent = config.type === 'agent-detail' && group.id !== 'agents' && group.id !== 'ungrouped' && group.id !== 'project settings' && (!isCollapsed || isMobile)

          return (
            <div key={group.id} className={shouldIndent ? 'ml-4' : ''}>
        ...
        <div className="border-t border-neutral-100 dark:border-neutral-800 p-3">
          {!mounted || !isLoaded ? (
            <div className={`flex items-center gap-3 ${isCollapsed && !isMobile ? 'justify-center' : ''}`}>
              <div className="w-8 h-8 bg-gray-200 dark:bg-neutral-700 rounded-full animate-pulse flex-shrink-0" />
              {(!isCollapsed || isMobile) && (
                <div className="min-w-0 flex-1">
                  <div className="w-16 h-3 bg-gray-200 dark:bg-neutral-700 rounded animate-pulse mb-1" />
                  <div className="w-12 h-2 bg-gray-200 dark:bg-neutral-700 rounded animate-pulse" />
                </div>
              )}
            </div>
          ) : (
            <SignedIn>
              <DropdownMenu>
                ...
                <DropdownMenuContent 
                  ...
                  className="w-56 shadow-lg border border-neutral-200 dark:border-neutral-700 rounded-xl bg-white dark:bg-neutral-800"
                >
